    ...
    describe("fromHex", () => {
      function doTest (data) {
        ...
        it(`fromHex(${data.hex}).hex => ${data.hex}`, () => {
          equal(Bc64.fromHex(data.hex).hex, data.hex)
        })
        it(`fromHex(${data.hex}).dhex => ${data.dhex}`, () => {
          equal(Bc64.fromHex(data.hex).dhex, data.dhex)
        })
        it(`fromHex(${data.hex}).b64Url => ${data.b64Url}`, () => {
          equal(Bc64.fromHex(data.hex).b64Url, data.b64Url)
        })

        // provide dhex : expect hex, dhex, b64Url
        it(`fromHex(${data.dhex}).hex => ${data.hex}`, () => {
          equal(Bc64.fromHex(data.dhex).hex, data.hex)
        })
        it(`fromHex(${data.dhex}).dhex => ${data.dhex}`, () => {
          equal(Bc64.fromHex(data.dhex).dhex, data.dhex)
        })
        it(`fromHex(${data.dhex}).b64Url => ${data.b64Url}`, () => {
          equal(Bc64.fromHex(data.dhex).b64Url, data.b64Url)
        })

        // provide hexs : expect hex, dhex, b64Url
        it(`fromHex(${data.hexs}).hex => ${data.hex}`, () => {
          equal(Bc64.fromHex(data.hexs).hex, data.hex)
        })
        it(`fromHex(${data.hexs}).dhex => ${data.dhex}`, () => {
          equal(Bc64.fromHex(data.hexs).dhex, data.dhex)
        })
        it(`fromHex(${data.hexs}).b64Url => ${data.b64Url}`, () => {
          equal(Bc64.fromHex(data.hexs).b64Url, data.b64Url)
        })
      }
    ...
    describe("fromB64", () => {
      function doTest (data) {
        ...
        it(`fromB64(${data.b64}).hex => ${data.hex}`, () => {
          equal(Bc64.fromB64(data.b64).hex, data.hex)
        })
        it(`fromB64(${data.b64}).dhex => ${data.dhex}`, () => {
          equal(Bc64.fromB64(data.b64).dhex, data.dhex)
        })
        it(`fromB64(${data.b64}).b64Url => ${data.b64Url}`, () => {
          equal(Bc64.fromB64(data.b64).b64Url, data.b64Url)
        })

        // provide b64Url : expect hex, dhex, b64Url
        it(`fromB64(${data.b64Url}).hex => ${data.hex}`, () => {
          equal(Bc64.fromB64(data.b64Url).hex, data.hex)
        })
        it(`fromB64(${data.b64Url}).dhex => ${data.dhex}`, () => {
          equal(Bc64.fromB64(data.b64Url).dhex, data.dhex)
        })
        it(`fromB64(${data.b64Url}).b64Url => ${data.b64Url}`, () => {
          equal(Bc64.fromB64(data.b64Url).b64Url, data.b64Url)
        })
